angular
  .module('core.sdk')
  .service('SavvyEssentialsSDK', function ($q, $log, SavvyEssentialsAPI, localStorageService, ValidationAPI) {
    this.getSavvyEssentials = function (date, local) {
      local = angular.isUndefined(local) ? false : local;
      var deferred = $q.defer();
      var _localStrage = 'api/savvyessentials';
      var packet = localStorageService.get(_localStrage);
      if (ValidationAPI.hasExpired(packet) || !local) {
        SavvyEssentialsAPI.savvyessentials().save({}, {currentDate: date}).$promise.then(
          function (response) {
      ...
      var _log = 'BlogSDK:getSavvyEssentials: ' + _localStrage;
      $log.debug(_log);
